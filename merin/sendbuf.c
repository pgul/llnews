...
#ifdef TTL
  opt=TTL;
  if (setsockopt(sockfd, /* IPPROTO_IP */ SOL_IP, IP_TTL, &opt, sizeof(opt)))
...
waitack:
  ...
  close(ack_sockfd);
  if (remote)
    syslog(LOG_INFO, "ACK for %s (stream %u) from %s (%s)", fname,
