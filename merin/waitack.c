...
//  syslog(LOG_DEBUG, "receive packet (ack?)");
  if (r<5*4+1 || buf[r-1])
    return 2;
  if (*(long *)buf!=htonl(MAGIC_ACK) || r!=strlen(buf+4*5)+4*5+1)
    return 2;
...
sendagain:
  ...
  remote=gethostbyaddr((char *)&down_addr.sin_addr, sizeof(down_addr.sin_addr),
                       AF_INET);
  if (remote)
    syslog(LOG_DEBUG, "Received ACK for %s (stream %u) from %s (%s)", buf+4*6,
           ll_stream, remote->h_name, ipaddr);
  else
    syslog(LOG_DEBUG, "Received ACK for %s (stream %u) from %s", buf+4*6,
           ll_stream, ipaddr);
  exit(0);
}
