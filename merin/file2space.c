...
nexttry:
  syslog(LOG_INFO, "Sending %s stream %u attempt %u to %s",
         file_name, ll_stream, tries, argv[2]);
  if (open_socket(argv[2])) return 2;
  p=(long *)buf;
  *p++=htonl(MAGIC_START);
  *p++=htonl(file_len);
  *p++=htonl(file_type);
...
//  printf("Sent, waiting for ack...\n");
  if (open_ack_socket(ll_stream)) return 2;
  if (waitack(file_name, file_len, file_ctime, file_type, ll_stream))
  {
    if (++tries==TRIES)
    {
      syslog(LOG_ERR, "No ACK for %s (stream %u)", file_name, ll_stream);
      return 1;
    }
    if (fromfile)
      fseek(fin, 0, SEEK_SET);
    syslog(LOG_NOTICE, "No ACK for %s (stream %u), retrying", file_name, ll_stream);
    goto nexttry;
  }
  else
    return 0;
